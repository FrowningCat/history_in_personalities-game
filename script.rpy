...
label disagreeWithThirdRequirement:
    ...
    show peten evil
    p "Я маршал Франции, и как главнокомандующий я не могу принять столь ужасные требования. Нет, это невозможно!"
...
label himmlerEnters:
    ...
    g "Здраствуйте мой фюрер. Чем могу быть полезен?"

    show himmler

    h "Помоги убедить нашего друга в пользе нашего сотрудничества."

    g "Дорогой маршал, давайте не будем сориться. Присоеденить к нам и мы приумножим коллониальные владения Франции."
...
label toBeRudeHimmler:
    ...
    g "Да как ты смеешь! Я цвет нашей нации! Мой фюрер, позвольте мне убить этого человека."
...
label toDefendThePosition:
    ...
    g "Подумайте еще раз, я уверен мы сможем найти общий язык. Вы, как ..."
...
label bendOver:
    ...
    h "Вот и славно. Я знал что вы благоразумный человек."
...
label argue:
    ...
    g "Aufhoert! Прекратите, вести себя так, будто это вы победили меня. Мы здесь совершенно по другому поводу. Вы обязаны подписать капитуляцию на моих условиях! Иначе ..."

    p "Иначе что? Я единственный кто способен сохранять порядок во франции, и вы это должн прекрасно понимать. Если не я ..."

    g "Если не вы, тогда ваше место займет кто-то другой. Я уверен что у нас получится найти подходящего человека."
    ...
    menu:
        ...
        "Отсаивать свою позицию":
            ...
            p "Без моего авторитета Великобритания захватит весь французский флот, что стоит в ее колониальных владениях, кроме того, огромное множество французов ступить в ее войска. Вам это нужно?"
...
label theEndingTwo:
    ...
    "Переговоры прошли ужасно, Вишиский режим стал полностью контролироваться из Германии. Маршал не смог перенести столь ужасного исхода, через два дня его нашли застреленным в собственном доме."
...
label theEndingThree:
    ...
    "Во время встречи Петен и Гитлера в Монтуа, первый не только отказался воевать с англичанами, но и смог убедить фюрера в том, что южная часть франции должна остаться независимой, дабы флот, стоявший в британских портах и колонии, не перешли противнику."
...
label theEndingFour:
    ...
    "Филипп Петен был застрелен. Его место занял Пьер Лаваль, которому не удалось отстоять свободную францию хоть в каком то виде из-за чего третий по численности флот мира был почти полностью конфискован Третьим Рейхом, а сама Франция была полностью оккупирована. "
